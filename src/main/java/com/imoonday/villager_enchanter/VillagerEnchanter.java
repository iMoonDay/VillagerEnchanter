...
public class VillagerEnchanter implements ModInitializer {
    ...
    public static final RegistryKey<PointOfInterestType> ENCHANTER_TYPE = RegistryKey.of(Registry.POINT_OF_INTEREST_TYPE.getKey(), ENCHANTER_ID);
    public static final VillagerProfession ENCHANTER = Registry.register(Registry.VILLAGER_PROFESSION, ENCHANTER_ID, new VillagerProfession("enchanter", entry -> entry.matchesKey(ENCHANTER_TYPE), entry1 -> entry1.matchesKey(ENCHANTER_TYPE), ImmutableSet.of(), ImmutableSet.of(), SoundEvents.ENTITY_VILLAGER_WORK_LIBRARIAN));
    public static final Item ENCHANT_REFRESH_BOOK = register(id("enchant_refresh_book"), new EnchantRefreshBookItem(new FabricItemSettings().maxCount(16).group(ItemGroup.MISC)));
